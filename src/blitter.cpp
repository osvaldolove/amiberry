...
static void blitter_dump(void)
{
	...
	write_log(_T("AFWM=%04X ALWM=%04X MOD A=%04X B=%04X C=%04X D=%04X\n"),
		blt_info.bltafwm,
		blt_info.bltalwm,
		blt_info.bltamod & 0xffff,
		blt_info.bltbmod & 0xffff,
		blt_info.bltcmod & 0xffff,
		blt_info.bltdmod & 0xffff);
...
static void blitter_done(int hpos)
{
	...
	if (log_blitter & 1)
		write_log(_T("cycles %d, missed %d, total %d\n"),
			...
			blit_totalcyclecounter + blit_misscyclecounter);
...
//			debugtest(DEBUGTEST_BLITTER,
//				_T("weird blt_info.hblitsize in linemode: %d vsize=%d\n"),
//				blt_info.hblitsize,
//				blt_info.vblitsize);
//			if (log_blitter & 16)
//				activate_debugger();
	...
	if (log_blitter & 1) {
		...
		if (1) {
			...
			write_log(_T("blitstart: %dx%d ch=%d %d*%d=%d d=%d f=%02X n=%d pc=%p l=%d dma=%04X %s\n"),
				...
				blitdesc ? 1 : 0,
				blitfill,
				dmaen(DMA_BLITPRI) ? 1 : 0,
				M68K_GETPC,
				blitline,
				dmacon,
				((dmacon & (DMA_MASTER | DMA_BLITTER)) == (DMA_MASTER | DMA_BLITTER)) ? _T("") : _T(" off!"));
			blitter_dump();
		}
	}
