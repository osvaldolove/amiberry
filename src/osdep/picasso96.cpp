...
*/
static void do_fillrect_frame_buffer(struct RenderInfo *ri,
	int X,
	int Y,
	int Width,
	int Height,
...
static void init_picasso_screen(void)
{
	...
	if (set_gc_called) {
		gfx_set_picasso_modeinfo(picasso96_state.Width,
			picasso96_state.Height,
...
STATIC_INLINE int BlitRect(uaecptr ri,
	uaecptr dstri,
	unsigned long srcx,
	unsigned long srcy,
	unsigned long dstx,
...
static void PlanarToChunky(struct RenderInfo *ri,
	struct BitMap *bm,
	unsigned long srcx,
	unsigned long srcy,
	unsigned long dstx,
...
/* NOTE: Watch for those planeptrs of 0x00000000 and 0xFFFFFFFF for all zero / all one bitmaps !!!! */
static void PlanarToDirect(struct RenderInfo *ri,
	struct BitMap *bm,
	unsigned long srcx,
	unsigned long srcy,
	unsigned long dstx,
...
bool picasso_flushpixels(uae_u8 *src, int off)
{
	...
	for (;;) {
		...
		if (dofull) {
			if (flashscreen != 0)
				copyallinvert(src + off,
					dst,
					pwidth,
					pheight,
					picasso96_state.BytesPerRow,
